import numpy,Slope_aspect,dist,time,rpy,Image,pickle,HistogramEqualization,pylab
...
def VegetationClassify(): 
  ...
  print "Creating Decision tree for predicting ladcover class"
  ...
  # Read the test data 
  Elev_arr = numpy.load("DEM.npy")
  River_arr = numpy.load("River.npy")
  ...
  # Apply histogram equalization on data 
  #Elev_arr = HistogramEqualization.HistEqualization(Elev_arr)
  ...
  #Image.fromarray(Veg_arr).show()
  numpy.save("landcover.npy",Veg_arr)
  pylab.imsave("Output/Landcover",Veg_arr)
